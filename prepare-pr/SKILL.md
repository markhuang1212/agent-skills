...
**If already on a `feature/` branch:** stay on it. No branch switch needed.

**If on any other non-main branch:** use it as-is (treat it like a feature branch).

## Step 3 — Stage and commit

Stage specific files rather than `git add -A` to avoid accidentally including `.env`,
large binaries, or unrelated scratch files. Review `git status` output and stage only
files that are clearly part of the finished work.
...
- If `git status` shows untracked files that look sensitive (`.env`, `*.key`, `*secret*`),
  skip them and mention it to the user.
